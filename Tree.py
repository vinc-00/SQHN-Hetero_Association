...
'''
    in_dim: Input dimension (e.g., 28 for MNIST).

    chnls: Number of channels (feature detectors) per layer.

    n_iter: Number of inference iterations.

    lr: Learning rate.

    actFunc: Activation function (0=max, 1=softmax, 2=mean-shifted).

    simFunc: Similarity function (0=dot, 1=cosine, 2=mean-shifted cosine, etc.).

    wt_up: Weight update rule (0=online avg, 1=weighted avg, 2=overwrite, 3=SGD).

    arch: Architecture variant (0-10, for different tasks).

    lmbd: Controls feedback strength (mixing bottom-up & top-down signals).

    gamma: Threshold for recognition.
'''
